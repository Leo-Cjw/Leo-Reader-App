...
function diagnosticDetail(entry: DiagnosticEntry) {
  ...
  if (entry.event === 'backup_created') return `${details.automatic ? '自动恢复点' : details.encrypted ? '口令加密' : '本机明文'} · ${formatBytes(Number(details.byteSize) || 0)}`;
...
function DataSafetyModal({ backups, automaticBackup, migrationSnapshots, health, pendingRestore, busy, onClose, onCheck, onRepair, onDiagnostics, onToggleAutomatic, onCreate, onScheduleRestore, onScheduleSnapshotRestore, onCancelRestore }: { backups: Backup[]; automaticBackup: AutomaticBackupStatus | null; migrationSnapshots: MigrationSnapshot[]; health: DataHealth | null; pendingRestore: PendingRestore | null; busy: boolean; onClose: () => void; onCheck: () => void; onRepair: () => void; onDiagnostics: () => void; onToggleAutomatic: (enabled: boolean) => void; onCreate: (passphrase?: string) => void; onScheduleRestore: (file: File, passphrase?: string) => void; onScheduleSnapshotRestore: (snapshot: MigrationSnapshot) => void; onCancelRestore: () => void }) {
    ...
    <div className="safety-grid">
      <section className="safety-section"><header><span><strong>资料库与备份</strong><small>先检查，再创建可验证恢复点</small></span></header>
        ...
        <div className="automatic-backup-card">
          <label className="settings-toggle automatic-backup-toggle">
            <input type="checkbox" checked={automaticBackup?.enabled || false} disabled={busy || !automaticBackup || Boolean(pendingRestore)} onChange={(event) => onToggleAutomatic(event.target.checked)}/>
            <span><strong>每日自动恢复点</strong><small>默认关闭。开启后每 24 小时在本机创建一份明文完整备份，仅保留最近 3 份自动恢复点。</small></span><i></i>
          </label>
          <p>{automaticBackup?.last_error_at
            ? `最近一次自动创建失败（${new Date(automaticBackup.last_error_at).toLocaleString('zh-CN')}）；Reader 会在条件恢复后重试。`
            : automaticBackup?.last_backup_at
              ? `最近：${new Date(automaticBackup.last_backup_at).toLocaleString('zh-CN')} · 已保留 ${automaticBackup.retained_count}/${automaticBackup.retention} 份`
              : automaticBackup?.enabled ? '尚无自动恢复点；无导入、休眠、低电量或恢复锁时会创建。' : '不会保存备份口令；手动、加密、修复前和恢复前备份不会被自动清理。'}</p>
        </div>
        ...
        <div className="backup-list">{backups.length ? backups.map((backup) => <article key={backup.id}><span className={`backup-mark ${backup.encrypted ? 'encrypted' : backup.automatic ? 'automatic' : ''}`} title={backup.encrypted ? '口令加密备份' : backup.automatic ? '自动恢复点' : '明文备份'}>{backup.encrypted ? 'E' : backup.automatic ? 'A' : 'B'}</span><span><strong>{formatDate(backup.created_at)} 的{backup.automatic ? '自动恢复点' : '备份'}</strong><small>{formatBytes(backup.byte_size)} · {backup.encrypted ? 'AES-256-GCM 加密' : '本机明文文件'} · {backup.verified_at ? '创建时已校验' : '恢复时校验'}</small></span><a className="button" href={`/api/backups/${backup.id}/download`}>下载</a></article>) : <div className="empty-state compact"><strong>还没有备份</strong><span>创建首个可验证恢复点。</span></div>}</div>
      </section>
...
function ReaderWorkspace() {
  ...
  const openSafety = async () => { setSafetyOpen(true); setBusySafety(true); try { const [result, snapshots, health] = await Promise.all([api.listBackups(), api.listMigrationSnapshots(), api.checkDataHealth()]); setBackups(result.backups); setAutomaticBackup(result.automaticBackup); setMigrationSnapshots(snapshots); setDataHealth(health); setPendingRestore(result.pendingRestore); } catch (error) { notify(error instanceof Error ? error.message : '数据安全信息加载失败', 'error'); } finally { setBusySafety(false); } };
  const toggleAutomaticBackups = async (enabled: boolean) => {
    setBusySafety(true);
    try {
      setAutomaticBackup(await api.updateAutomaticBackups(enabled));
      const next = await api.listBackups();
      setBackups(next.backups);
      setAutomaticBackup(next.automaticBackup);
      notify(enabled ? next.automaticBackup.last_backup_at ? '自动恢复点已开启，首份恢复点已创建' : '自动恢复点已开启，将在系统条件允许时创建' : '自动恢复点已关闭；现有恢复点会保留');
    } catch (error) { notify(error instanceof Error ? error.message : '自动恢复点设置失败', 'error'); }
