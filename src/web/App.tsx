...
import { api } from './api';
import type { AIProvenance, AISettings, AIStatus, Article, ArticleRevision, ArticleRevisionSummary, Attachment, Backup, Collection, ConnectorStatus, DuplicateGroup, Highlight, HighlightColor, ImportJob, PendingRestore, RAGCitation, SmartCollection, SmartCollectionRule, Source, Stats, SummaryResult, Tag, View } from './types';
...
type DesktopCommand = 'new' | 'search' | 'edit' | 'settings' | 'data-safety' | 'toggle-ai';
...
function Sidebar({ view, setView, collectionId, setCollectionId, smartCollectionId, setSmartCollectionId, collections, smartCollections, tags, tagFilter, setTagFilter, stats, sources, onAdd, onSources, onCollections, onSmartCollections, onDuplicates, onDataSafety, onMoveArticles }: {
  ...
  return <aside className="sidebar" data-screen-label="产品导航">
    ...
    <nav className="nav-stack" aria-label="资料库">
      {items.map((item) => <button key={item.view} type="button" className={`nav-item ${view === item.view && !collectionId && !smartCollectionId && !tagFilter ? 'active' : ''}`} onClick={() => { setView(item.view); setCollectionId(null); setSmartCollectionId(null); setTagFilter(''); }}>
    ...
    <nav className="nav-stack smart-collection-nav" aria-label="智能资料夹">
      {smartCollections.length ? smartCollections.map((collection) => <button key={collection.id} type="button" title={describeSmartRule(collection.rule)} className={`nav-item ${smartCollectionId === collection.id ? 'active' : ''}`} onClick={() => { setSmartCollectionId(collection.id); setCollectionId(null); setView('inbox'); setTagFilter(''); }}>
    ...
    {tags.length > 0 && <><div className="section-heading"><span>标签</span></div><nav className="nav-stack tag-nav" aria-label="标签">{tags.slice(0, 6).map((tag) => <button key={tag.id} type="button" className={`nav-item ${tagFilter === tag.name ? 'active' : ''}`} onClick={() => { setTagFilter(tag.name); setCollectionId(null); setSmartCollectionId(null); setView('inbox'); }}><span className="nav-glyph">#</span><span className="nav-label">{tag.name}</span><span className="nav-count">{tag.article_count}</span></button>)}</nav></>}
...
function AIPanel({ article, onClose, onArticleUpdated, onDerivedCreated, onOpenCitation, configurationVersion, notify }: { article: Article | null; onClose: () => void; onArticleUpdated: (article: Article) => void; onDerivedCreated: (article: Article, message: string) => void; onOpenCitation: (citation: RAGCitation) => void; configurationVersion: number; notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  return <aside className="ai-panel" data-screen-label="AI 文章助手">
    ...
    <div className="ai-tabs"><button type="button" className={tab === 'summary' ? 'active' : ''} onClick={() => setTab('summary')}>摘要</button><button type="button" className={tab === 'chat' ? 'active' : ''} onClick={() => setTab('chat')}>对话</button><button type="button" className={tab === 'translate' ? 'active' : ''} onClick={() => setTab('translate')}>翻译</button></div>
...
function AISettingsModal({ onClose, onConfigurationChanged, notify }: { onClose: () => void; onConfigurationChanged: (status: AIStatus) => void; notify: (message: string, tone?: Toast['tone']) => void }) {
  const [settings, setSettings] = useState<AISettings | null>(null);
  const [enabled, setEnabled] = useState(false);
  const [endpoint, setEndpoint] = useState('');
  const [apiKey, setApiKey] = useState('');
  const [clearApiKey, setClearApiKey] = useState(false);
  const [busy, setBusy] = useState<'load' | 'save' | 'test' | 'reset' | null>('load');
  const [testResult, setTestResult] = useState<{ ok: boolean; message: string } | null>(null);
  useEffect(() => {
    void api.getAISettings().then((value) => { setSettings(value); setEnabled(value.enabled); setEndpoint(value.endpoint); }).catch((error) => notify(error instanceof Error ? error.message : 'AI 设置加载失败', 'error')).finally(() => setBusy(null));
  }, [notify]);
  const save = async () => {
...
function AddModal({ collections, onClose, onCreated, onQueued, notify, onSourceCreated, onOpenConnectors }: { collections: Collection[]; onClose: () => void; onCreated: (article: Article) => void; onQueued: (job: ImportJob) => void; notify: (message: string, tone?: Toast['tone']) => void; onSourceCreated: (source: Source) => void; onOpenConnectors: () => void }) {
  const [tab, setTab] = useState<'url' | 'attachment' | 'markdown' | 'feed'>('url');
  const [url, setURL] = useState(''); const [title, setTitle] = useState(''); const [content, setContent] = useState(''); const [file, setFile] = useState<File | null>(null); const [collection, setCollection] = useState('inbox'); const [busy, setBusy] = useState(false);
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal" role="dialog" aria-modal="true" aria-label="添加内容">
    <header><div><span className="eyebrow">本地采集</span><h2>添加到 Reader</h2></div><button className="icon-button" type="button" aria-label="关闭添加窗口" onClick={onClose}>×</button></header>
    <div className="modal-tabs">{([['url','网页 URL'],['attachment','附件'],['markdown','Markdown'],['feed','自动订阅']] as const).map(([value,label]) => <button type="button" key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}>{label}</button>)}</div>
    <div className="modal-body">
      ...
      {tab === 'attachment' && <label className="file-drop"><input type="file" accept=".pdf,.png,.jpg,.jpeg,.gif,.webp,.avif,.heic,.mp4,.mov,.m4v,.webm,.txt,.md,.markdown" onChange={(event) => setFile(event.target.files?.[0] || null)}/><span className="file-drop-icon">＋</span><strong>{file ? file.name : '选择 PDF、图片、视频或文本'}</strong><small>{file ? `${formatBytes(file.size)} · ${file.type || '未知类型'}` : '单个文件最大 100 MB，原文件和内容都只保存在本机。'}</small></label>}
      ...
      {tab !== 'feed' && <label><span>保存到</span><select value={collection} onChange={(event) => setCollection(event.target.value)}>{collections.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}
    ...
    <footer><button className="button" type="button" onClick={onClose}>取消</button><button className="button primary" type="button" disabled={busy} onClick={() => void submit()}>{busy ? '正在处理…' : tab === 'feed' ? '添加并同步' : tab === 'markdown' ? '保存到本机' : '加入导入队列'}</button></footer>
  </section></div>;
}
  ...
  return <div className="modal-backdrop smart-collections-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal smart-collections-modal" role="dialog" aria-modal="true" aria-label="智能资料夹管理">
    ...
    <div className="smart-collections-workspace">
      ...
      <form className="smart-rule-form" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
        <div className="smart-rule-intro"><span className="eyebrow">{editing ? '编辑动态视图' : '新建动态视图'}</span><h3>{editing?.name || '把常用筛选保存下来'}</h3><div className="match-switch" role="group" aria-label="规则匹配方式"><button type="button" className={rule.match === 'all' ? 'active' : ''} onClick={() => setRulePatch({ match: 'all' })}>全部满足</button><button type="button" className={rule.match === 'any' ? 'active' : ''} onClick={() => setRulePatch({ match: 'any' })}>任一满足</button></div></div>
...
function ImportQueueModal({ jobs, onClose, onRetry }: { jobs: ImportJob[]; onClose: () => void; onRetry: (job: ImportJob) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal queue-modal" role="dialog" aria-modal="true" aria-label="导入队列">
    ...
    <div className="queue-summary"><strong>{jobs.filter((job) => job.status === 'pending' || job.status === 'running').length}</strong><span>个任务正在处理。应用重启后，未完成任务会自动继续。</span></div>
...
function VersionHistoryModal({ article, revisions, preview, busy, onClose, onSelect, onRestore }: { article: Article; revisions: ArticleRevisionSummary[]; preview: ArticleRevision | null; busy: boolean; onClose: () => void; onSelect: (version: number) => void; onRestore: (version: number) => void }) {
  ...
  return <div className="modal-backdrop history-backdrop"><section className="modal history-modal" role="dialog" aria-modal="true" aria-label="版本历史">
    ...
    <div className="history-workspace">
      <aside className="revision-list"><div className="history-summary"><strong>{revisions.length}</strong><span>个本地版本</span></div>{revisions.map((revision) => <button key={revision.id} type="button" className={preview?.version === revision.version ? 'active' : ''} onClick={() => onSelect(revision.version)}>
...
function DataSafetyModal({ backups, pendingRestore, busy, onClose, onCreate, onScheduleRestore, onCancelRestore }: { backups: Backup[]; pendingRestore: PendingRestore | null; busy: boolean; onClose: () => void; onCreate: (passphrase?: string) => void; onScheduleRestore: (file: File, passphrase?: string) => void; onCancelRestore: () => void }) {
  ...
  return <div className="modal-backdrop safety-backdrop"><section className="modal safety-modal" role="dialog" aria-modal="true" aria-label="数据安全中心">
    ...
    <div className="safety-grid">
      <section className="safety-section"><header><span><strong>完整备份</strong><small>SQLite、附件和校验清单</small></span></header>
    ...
    <footer className="safety-footer"><span>恢复不会立即覆盖数据；Reader 会先创建安全备份，并在下次启动时原子替换。</span></footer>
  </section></div>;
}
...
export function App() {
  ...
  const jobPollInterval = jobs.some((job) => job.status === 'pending' || job.status === 'running') ? 800 : 5000;
  ...
  const openSafety = async () => { setSafetyOpen(true); setBusySafety(true); try { const result = await api.listBackups(); setBackups(result.backups); setPendingRestore(result.pendingRestore); } catch (error) { notify(error instanceof Error ? error.message : '备份列表加载失败', 'error'); } finally { setBusySafety(false); } };
  ...
  return <div className="app-stage" data-theme={theme} data-desktop={isDesktop ? 'true' : 'false'}>
    ...
    {addOpen && <AddModal collections={collections} onClose={() => setAddOpen(false)} onCreated={created} onQueued={queued} notify={notify} onSourceCreated={(source) => { setSources((current) => [...current, source]); void refreshChrome(); }} onOpenConnectors={() => { setAddOpen(false); setConnectorSettingsOpen(true); }}/>}
    ...
    {safetyOpen && <DataSafetyModal backups={backups} pendingRestore={pendingRestore} busy={busySafety} onClose={() => setSafetyOpen(false)} onCreate={(passphrase) => void createLocalBackup(passphrase)} onScheduleRestore={(file, passphrase) => void scheduleLocalRestore(file, passphrase)} onCancelRestore={() => void cancelLocalRestore()}/>}
