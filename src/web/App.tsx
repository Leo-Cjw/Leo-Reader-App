  ...
  return <div className="modal-backdrop safety-backdrop"><section className="modal safety-modal" role="dialog" aria-modal="true" aria-label="数据安全中心">
    ...
    <div className="safety-grid">
      <section className="safety-section"><header><span><strong>资料库与备份</strong><small>先检查，再创建可验证恢复点</small></span></header>
        ...
        <div className="backup-list">{backups.length ? backups.map((backup) => <article key={backup.id}><span className={`backup-mark ${backup.encrypted ? 'encrypted' : backup.automatic ? 'automatic' : ''}`} title={backup.encrypted ? '口令加密备份' : backup.automatic ? '自动恢复点' : '明文备份'}>{backup.encrypted ? 'E' : backup.automatic ? 'A' : 'B'}</span><span><strong>{formatDate(backup.created_at)} 的{backup.automatic ? '自动恢复点' : '备份'}</strong><small>{formatBytes(backup.byte_size)} · {backup.encrypted ? 'AES-256-GCM 加密' : '本机明文文件'}</small></span><a className="button" href={`/api/backups/${backup.id}/download`}>下载</a></article>) : <div className="empty-state compact"><strong>还没有备份</strong><span>创建首个可验证恢复点。</span></div>}</div>
      </section>
