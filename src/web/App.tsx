...
type DesktopCommand = 'new' | 'search' | 'edit' | 'settings' | 'import-queue' | 'data-safety' | 'toggle-ai';
...
function AISettingsModal({ notificationsAvailable, onClose, onConfigurationChanged, notify }: { notificationsAvailable: boolean; onClose: () => void; onConfigurationChanged: (status: AIStatus) => void; notify: (message: string, tone?: Toast['tone']) => void }) {
  ...
  const [busy, setBusy] = useState<'load' | 'save' | 'test' | 'reset' | 'notifications' | null>('load');
  ...
  useEffect(() => {
    void Promise.all([
      ...
      setSettings(value); setEnabled(value.enabled); setEndpoint(value.endpoint); setNotificationSettings(notifications);
  ...
  const toggleNotifications = async (nextEnabled: boolean) => {
    ...
    try {
      const next = await api.updateNotificationSettings(nextEnabled);
      setNotificationSettings(next);
      notify(next.enabled ? '后台导入通知已开启' : '后台导入通知已关闭');
