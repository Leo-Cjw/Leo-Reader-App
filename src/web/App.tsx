...
function DataSafetyModal({ backups, migrationSnapshots, pendingRestore, busy, onClose, onCreate, onScheduleRestore, onCancelRestore }: { backups: Backup[]; migrationSnapshots: MigrationSnapshot[]; pendingRestore: PendingRestore | null; busy: boolean; onClose: () => void; onCreate: (passphrase?: string) => void; onScheduleRestore: (file: File, passphrase?: string) => void; onCancelRestore: () => void }) {
...
export function App() {
  ...
  const openSafety = async () => { setSafetyOpen(true); setBusySafety(true); try { const [result, snapshots] = await Promise.all([api.listBackups(), api.listMigrationSnapshots()]); setBackups(result.backups); setMigrationSnapshots(snapshots); setPendingRestore(result.pendingRestore); } catch (error) { notify(error instanceof Error ? error.message : '数据安全信息加载失败', 'error'); } finally { setBusySafety(false); } };
