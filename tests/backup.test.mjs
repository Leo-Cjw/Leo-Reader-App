...
test('migration snapshot restore preserves files, backs up current data and remigrates on restart', async (t) => {
  ...
  assert.deepEqual(
    ...
    { kind: 'migration_snapshot', snapshotId: snapshot.id, from: 8, to: 10, hasPrivatePath: true, hash: true }
  );
  ...
  const restored = await new ReaderDatabase(dbPath).initialize();
  assert.equal((await restored.one('SELECT max(version) AS version FROM schema_migrations;')).version, 10);
...
test('migration snapshot restore rejects incompatible or changed snapshots before replacement', async (t) => {
  ...
  const snapshotPath = path.join(root, 'reader-before-schema-v8-to-v11-2026-01-01T00-00-00-000Z-00000000-0000-4000-8000-000000000000.sqlite3');
  ...
  const compatiblePath = path.join(migrationDirectory, 'reader-before-schema-v8-to-v10-2026-01-01T00-00-00-000Z-11111111-1111-4111-8111-111111111111.sqlite3');
