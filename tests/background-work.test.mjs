...
test('background policy combines restore, sleep, connectivity and power constraints without premature resume', async () => {
  ...
  const backups = worker();
  const policy = createBackgroundWorkPolicy(imports, sources, semantic, backups);
  ...
  assert.equal(backups.resumes, 0);
  ...
  assert.equal(backups.resumes, 1);
  ...
  assert.equal(policy.snapshot().automaticBackupsPaused, false);
  assert.deepEqual(policy.snapshot().automaticBackupPauseReasons, []);
});
