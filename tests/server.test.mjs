...
test('migration snapshots are listed without private paths and can be exported safely', async (t) => {
  ...
  assert.deepEqual(
    ...
    { from: 7, to: SCHEMA_VERSION, privatePath: undefined }
  );
  ...
  assert.match(download.headers.get('content-disposition') || '', new RegExp(`reader-before-schema-v7-to-v${SCHEMA_VERSION}`));
  ...
  assert.deepEqual(
    ...
    { kind: 'migration_snapshot', snapshotId: snapshot.id, from: 7, to: SCHEMA_VERSION, restartRequired: true, privatePath: undefined, privateHash: undefined }
  );
