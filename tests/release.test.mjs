...
test('macOS release metadata requires one numeric version and build identity for every bundle', () => {
  const metadata = releaseBundleMetadata({ version: '0.60.0', build: { buildVersion: '60' } });
  assert.deepEqual(metadata, { version: '0.60.0', buildVersion: '60' });
  ...
  assert.throws(() => releaseBundleMetadata({ version: '0.60.0', build: { buildVersion: '0' } }), /正整数/);
...
function fakeReleaseMetadata(version = '0.60.0', buildVersion = '60') {
  return {
    version,
    build: {
      buildVersion,
...
test('ad-hoc release manifest atomically records and verifies only the DMG identity', async () => {
  ...
  const dmgName = 'Reader-0.60.0-universal.dmg';
...
test('release manifest verification rejects artifact tampering', async () => {
  ...
  const dmgPath = path.join(temporary, 'Reader-0.60.0-universal.dmg');
...
test('notarized release manifest requires a clean source and both distributable artifacts', async () => {
  ...
  await writeFile(path.join(temporary, 'Reader-0.60.0-universal.dmg'), 'notarized-reader-dmg');
  await writeFile(path.join(temporary, 'Reader-0.60.0-darwin-universal.zip'), 'notarized-reader-update');
